...
<div class="space-y-6">
    ...
    <div class="border-b border-walnut-800/10 pb-6">
        ...
        <h1 class="text-3xl font-black uppercase tracking-tight text-walnut-950 mt-2">Log Aktivitas</h1>
    ...
    @if($logs->isEmpty())
        <div class="bg-cream-50 border border-walnut-800/10 rounded-[32px] p-12 text-center text-muted">
            <i data-lucide="history" class="w-10 h-10 text-walnut-300 mx-auto mb-3"></i>
                        ...
                        <tr class="border-b border-walnut-800/10 last:border-0 hover:bg-cream-100/50">
                            <td class="px-5 py-4">
                                @if($log->user)
                                    ...
                                    <span class="text-[0.6rem] text-walnut-400 block">{{ $log->user->email }}</span>
                                @else
                                    <span class="text-walnut-400 font-semibold text-xs">Sistem</span>
                                @endif
                            </td>
                            <td class="px-5 py-4">
                                <span class="inline-block text-[0.6rem] font-mono font-black uppercase bg-gold-50 text-gold-700 px-2 py-0.5 rounded">
                                    {{ $log->action }}
                                </span>
                            </td>
                            <td class="px-5 py-4 text-walnut-800 text-xs font-medium">
                                ...
                                @if($log->model_type && $log->model_id)
                                    <span class="block text-[0.55rem] text-walnut-400 font-mono mt-0.5">Model: {{ class_basename($log->model_type) }} #{{ $log->model_id }}</span>
                                @endif
                            </td>
                            <td class="px-5 py-4 min-w-[150px]">
                                <span class="text-[0.65rem] text-walnut-600 font-bold block">IP: {{ $log->ip_address }}</span>
                                <span class="text-[0.55rem] text-walnut-400 font-medium line-clamp-1" title="{{ $log->user_agent }}">{{ $log->user_agent }}</span>
                            </td>
