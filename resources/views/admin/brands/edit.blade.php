...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="border-b border-walnut-800/10 pb-6 flex items-center justify-between">
        <div>
            ...
            <h1 class="text-3xl font-black uppercase tracking-tight text-walnut-950 mt-2">Edit Brand</h1>
        </div>
    ...
    <div class="bg-cream-50 border border-walnut-800/10 rounded-[32px] p-8 shadow-sm">
        <form action="{{ route('admin.brands.update', $brand->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-1.5">
                <label for="name" class="text-[0.65rem] uppercase tracking-widest text-walnut-400 font-bold block">Nama Brand</label>
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="description" id="description" rows="4"
                    class="w-full px-5 py-4 bg-cream-100 border border-walnut-800/10 rounded-2xl text-walnut-900 focus:outline-none focus:ring-2 focus:ring-gold-500 focus:bg-cream-50 transition text-xs font-semibold leading-relaxed @error('description') border-rose-500 @enderror">{{ old('description', $brand->description) }}</textarea>
                @error('description')
                    <span class="text-[0.65rem] text-rose-600 font-semibold block mt-1">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Logo -->
                <div class="space-y-1.5">
                    <label for="logo" class="text-[0.65rem] uppercase tracking-widest text-walnut-400 font-bold block">Logo Brand</label>
                    @if($brand->logo)
                        <div class="mb-2 bg-cream-100 p-1 border border-walnut-800/10 rounded-lg inline-block w-16 h-16 flex items-center justify-center">
