...
@section('title', 'Kelola Brand - Admin MusicStore Luxe')
...
<div class="space-y-6">
    ...
    <div class="border-b border-slate-100 pb-6 flex items-center justify-between">
        <div>
            ...
            <h1 class="text-3xl font-black uppercase tracking-tight text-slate-950 mt-2">Kelola Brand</h1>
            <p class="text-xs text-slate-500">Kelola brand atau produsen instrumen musik yang tersedia.</p>
        </div>
        <a href="{{ route('admin.brands.create') }}" class="inline-flex items-center justify-center px-4 py-2.5 bg-indigo-600 rounded-xl text-xs font-semibold uppercase tracking-wider text-white hover:bg-indigo-700 hover:shadow-lg hover:shadow-indigo-600/10 transition duration-300">
    ...
    @if($brands->isEmpty())
        <div class="bg-white border border-slate-200/80 rounded-[32px] p-12 text-center text-slate-500">
            <i data-lucide="award" class="w-10 h-10 text-slate-350 mx-auto mb-3"></i>
                    ...
                    @foreach($brands as $brand)
                        <tr class="border-b border-slate-100 last:border-0 hover:bg-slate-50/50">
                            <td class="px-6 py-4 flex items-center gap-4">
                                <div class="w-10 h-10 bg-slate-50 rounded-lg overflow-hidden shrink-0 border border-slate-100 flex items-center justify-center p-1 bg-white">
                                    @if($brand->logo)
                                ...
                                <div>
                                    <span class="font-bold text-slate-900 block">{{ $brand->name }}</span>
                                    <span class="text-[0.65rem] text-slate-400 font-mono">slug: {{ $brand->slug }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-slate-500 text-xs truncate max-w-xs">
                                {{ $brand->description ?: '-' }}
                            </td>
                            <td class="px-6 py-4 font-bold text-slate-800">{{ $brand->products_count }} Produk</td>
                            ...
                            <td class="px-6 py-4 text-center space-x-3">
                                <a href="{{ route('admin.brands.edit', $brand->id) }}" class="text-xs font-bold text-indigo-650 hover:underline">Edit</a>
