...
@section('title', 'Kelola Kategori - Admin DjudasMS')
...
<div class="space-y-6">
    ...
    <div class="border-b border-walnut-800/10 pb-6 flex items-center justify-between">
        <div>
            ...
            <h1 class="text-3xl font-black uppercase tracking-tight text-walnut-950 mt-2">Kelola Kategori</h1>
            <p class="text-xs text-muted">Kelola kategori produk dan subkategori instrumen musik.</p>
        </div>
        <a href="{{ route('admin.categories.create') }}" class="inline-flex items-center justify-center px-4 py-2.5 bg-gold-600 rounded-xl text-xs font-semibold uppercase tracking-wider text-white hover:bg-gold-700 hover:shadow-lg hover:shadow-indigo-600/10 transition duration-300">
    ...
    @if($categories->isEmpty())
        <div class="bg-cream-50 border border-walnut-800/10 rounded-[32px] p-12 text-center text-muted">
            <i data-lucide="layers" class="w-10 h-10 text-walnut-300 mx-auto mb-3"></i>
                    ...
                    @foreach($categories as $category)
                        <tr class="border-b border-walnut-800/10 last:border-0 hover:bg-cream-100/50">
                            <td class="px-6 py-4 flex items-center gap-4">
                                <div class="w-10 h-10 bg-cream-100 rounded-lg overflow-hidden shrink-0 border border-walnut-800/10">
                                    @if($category->image)
                                ...
                                <div>
                                    <span class="font-bold text-walnut-900 block">{{ $category->name }}</span>
                                    <span class="text-[0.65rem] text-walnut-400 font-mono">slug: {{ $category->slug }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-muted font-semibold text-xs">
                                {{ $category->parent ? $category->parent->name : '-' }}
                            </td>
                            <td class="px-6 py-4 font-bold text-walnut-800">{{ $category->products_count }} Produk</td>
                            ...
                            <td class="px-6 py-4 text-center space-x-3">
                                <a href="{{ route('admin.categories.edit', $category->id) }}" class="text-xs font-bold text-gold-600 hover:underline">Edit</a>
