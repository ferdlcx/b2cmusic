...
@section('title', 'Kelola Flash Sale - Admin DjudasMS')
...
<div class="space-y-6">
    ...
    <div class="border-b border-walnut-800/10 pb-6 flex items-center justify-between">
        <div>
            ...
            <h1 class="text-3xl font-black uppercase tracking-tight text-walnut-950 mt-2">Kelola Flash Sale</h1>
            <p class="text-xs text-muted">Kelola promosi flash sale berbatas waktu untuk instrumen musik.</p>
        </div>
        <a href="{{ route('admin.flashSales.create') }}" class="inline-flex items-center justify-center px-4 py-2.5 bg-gold-600 rounded-xl text-xs font-semibold uppercase tracking-wider text-white hover:bg-gold-700 hover:shadow-lg hover:shadow-indigo-600/10 transition duration-300">
    ...
    @if($flashSales->isEmpty())
        <div class="bg-cream-50 border border-walnut-800/10 rounded-[32px] p-12 text-center text-muted">
            <i data-lucide="zap" class="w-10 h-10 text-walnut-300 mx-auto mb-3"></i>
                    ...
                    @foreach($flashSales as $sale)
                        <tr class="border-b border-walnut-800/10 last:border-0 hover:bg-cream-100/50">
                            ...
                            <td class="px-6 py-4 text-center space-x-3">
                                <a href="{{ route('admin.flashSales.edit', $sale->id) }}" class="text-xs font-bold text-gold-600 hover:underline">Edit</a>
