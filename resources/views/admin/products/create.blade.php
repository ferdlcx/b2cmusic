...
@section('title', 'Tambah Produk Baru - Admin DjudasMS')
...
<div class="max-w-4xl mx-auto space-y-10 py-4">
    ...
    <!-- Form -->
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="bg-cream-50 border border-walnut-800/10 rounded-[32px] p-8 md:p-10 shadow-sm space-y-8">
        @csrf
        ...
        <div class="grid gap-6 md:grid-cols-2">
            ...
            <div class="space-y-2">
                ...
                <select name="category_id" id="category_id" required
                    class="w-full px-5 py-3.5 bg-cream-100 border border-walnut-800/10 rounded-2xl text-walnut-900 focus:outline-none focus:ring-2 focus:ring-gold-500 focus:bg-cream-50 transition text-sm @error('category_id') border-rose-500 @enderror">
        ...
        <div class="space-y-2">
            <label for="image" class="text-xs uppercase tracking-widest text-muted font-bold block">Gambar Utama Produk</label>
            <input type="file" name="image" id="image" required
                class="w-full px-5 py-3.5 bg-cream-100 border border-walnut-800/10 rounded-2xl text-walnut-900 focus:outline-none focus:ring-2 focus:ring-gold-500 focus:bg-cream-50 transition text-sm @error('image') border-rose-500 @enderror" />
            @error('image')
                <span class="text-xs text-rose-600 font-medium block mt-1">{{ $message }}</span>
            @enderror
        </div>
