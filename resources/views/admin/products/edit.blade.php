...
@section('title', 'Edit Produk: ' . $product->name . ' - Admin MusicStore')
...
<div class="max-w-4xl mx-auto space-y-10 py-4">
    ...
    <div class="border-b border-slate-100 pb-8 flex items-center justify-between">
        <div>
            <span class="text-xs uppercase tracking-[0.45em] text-slate-500 font-bold">Menu Edit</span>
            <h1 class="text-4xl md:text-5xl font-black uppercase tracking-[-0.04em] text-slate-950 mt-3">Edit Produk</h1>
        </div>
    ...
    <!-- Form -->
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="bg-white border border-slate-200 rounded-[32px] p-8 md:p-10 shadow-sm space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-2">
            ...
            <textarea name="description" id="description" rows="5"
                class="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-950 focus:bg-white transition text-sm">{{ old('description', $product->description) }}</textarea>
        </div>

        <!-- Current & New Image Upload -->
        <div class="grid gap-6 md:grid-cols-[150px_1fr]">
            <div>
                <span class="text-xs uppercase tracking-widest text-slate-500 font-bold block mb-2">Gambar Saat Ini</span>
                <div class="w-full h-32 rounded-2xl overflow-hidden bg-slate-50 border border-slate-200 flex items-center justify-center">
            ...
            <div class="space-y-2 flex flex-col justify-end">
                <label for="image" class="text-xs uppercase tracking-widest text-slate-500 font-bold block">Ganti Gambar (Opsional)</label>
