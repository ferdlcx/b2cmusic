...
<div class="space-y-6">
    ...
    <div class="border-b border-walnut-800/10 pb-6">
        ...
        <h1 class="text-3xl font-black uppercase tracking-tight text-walnut-950 mt-2">Moderasi Ulasan</h1>
    ...
    @if($reviews->isEmpty())
        <div class="bg-cream-50 border border-walnut-800/10 rounded-[32px] p-12 text-center text-muted">
            <i data-lucide="message-square" class="w-10 h-10 text-walnut-300 mx-auto mb-3"></i>
                    ...
                    @foreach($reviews as $review)
                        <tr class="border-b border-walnut-800/10 last:border-0 hover:bg-cream-100/50">
                            <td class="px-6 py-5">
                                ...
                                <span class="text-[0.65rem] text-walnut-400 font-semibold block">{{ $review->user->email }}</span>
                                <span class="text-[0.65rem] text-gold-600 font-bold block mt-2 uppercase tracking-wide">Produk: {{ $review->product->name }}</span>
                            </td>
                            <td class="px-6 py-5 max-w-sm">
                                ...
                                <div class="flex items-center gap-0.5 text-gold-400 mb-1.5">
                                    @for($i = 1; $i <= 5; $i++)
                                        ...
                                        @else
                                            <i data-lucide="star" class="w-3.5 h-3.5 text-walnut-200"></i>
                                        @endif
                                    @endfor
                                </div>
                                <p class="text-xs text-walnut-800 font-medium leading-relaxed">{{ $review->comment }}</p>
                                <span class="text-[0.6rem] text-walnut-400 block mt-1">{{ $review->created_at->format('d M Y, H:i') }}</span>
                            </td>
                            <td class="px-6 py-5">
                                @if($review->photo)
                                    <div class="w-12 h-12 rounded-lg overflow-hidden border border-walnut-800/10 relative group shrink-0">
                                ...
                                @else
                                    <span class="text-xs text-walnut-400 font-semibold">-</span>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-5 text-center space-y-2">
                                ...
                                @else
                                    <span class="text-xs text-walnut-400 font-semibold">Moderasi Selesai</span>
                                @endif
                            </td>
                        </tr>
                    @endforeach
