                        ...
                        <tbody>
                            @foreach($cartItems as $item)
                                <tr class="border-b border-walnut-800/10 last:border-0 hover:bg-cream-50 transition">
                                    ...
                                    <!-- Quantity -->
                                    <td class="py-6 text-center">
                                        <form action="{{ route('cart.update', $item->id) }}" method="POST" x-data="{ qty: {{ $item->quantity }} }" x-ref="formDesktop" class="inline-flex items-center border border-walnut-800/20 w-32 bg-transparent">
                                            @csrf
                                            <button type="button" @click="if (qty > 1) { qty--; $nextTick(() => $refs.formDesktop.submit()) }" class="w-10 py-2 hover:text-gold-600 text-walnut-500 font-bold focus:outline-none transition">-</button>
                                            <input type="number" name="quantity" :value="qty" readonly class="flex-1 w-full min-w-0 bg-transparent text-center text-[0.75rem] font-bold text-walnut-950 focus:outline-none appearance-none m-0" />
                                            <button type="button" @click="if (qty < {{ $item->product->stock }}) { qty++; $nextTick(() => $refs.formDesktop.submit()) }" class="w-10 py-2 hover:text-gold-600 text-walnut-500 font-bold focus:outline-none transition">+</button>
                                        </form>
                                    </td>
                    ...
                    @foreach($cartItems as $item)
                        <div class="border-b border-walnut-800/10 pb-6 space-y-4">
                            ...
                            <div class="flex items-center justify-between pt-2">
                                <form action="{{ route('cart.update', $item->id) }}" method="POST" x-data="{ qty: {{ $item->quantity }} }" x-ref="formMobile" class="inline-flex items-center border border-walnut-800/20 w-28">
                                    @csrf
                                    ...
                                    <input type="number" name="quantity" :value="qty" readonly class="flex-1 w-full min-w-0 bg-transparent text-center text-[0.7rem] font-bold text-walnut-950 focus:outline-none appearance-none m-0" />
